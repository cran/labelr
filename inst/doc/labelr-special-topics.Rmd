...
airlines' carrier codes to their full ships with the nycflights13 package itself.
...
Let's add use these vectors to add value labels to df. We'll demo 
`add_val1()`, which accepts only on variable but allows you to pass its name 
unquoted. 
...
that shares its index. For example, in the above case, `ny_month_vals[3]` is 
(here, `3`) is  associated with the ny_month_labs[3] (here, `"MAR"`)).
